...
function TimeTable() {
   const { data, dataUpdatedAt, error, isError, isSuccess, isFetching } = useSuspenseQuery(options())
            ...
            <TableBody>
               {data.table.map((entry) => (
                  <TableRow key={entry.timing}>
                     ...
                     <TableCell className="text-center">{entry[day] !== "" ? entry[day] : "❌"}</TableCell>
                  </TableRow>
               ))}
            </TableBody>
            <TableFooter>
...
function options() {
   return queryOptions({
      queryKey: ["time-table"],
      queryFn: async () => {
         const html = await erp.get("https://erp.psit.ac.in/Student/MyTimeTable", auth.username, auth.password)
         ...
         const formattedData = {
            ...
            weeks: {
               object: data.weeks.reduce((prevObject, week, idx) => ({ ...prevObject, [idx + 1]: week.day }), {}),
