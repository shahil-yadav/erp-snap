...
import { AndroidRounded, Download } from "@mui/icons-material";
    ...
    >
      <div className="flex justify-between">
        ...
        <Link href={release.downloadUrl}>
          <IconButton color={release.isLatest ? "primary" : "error"}>
            <Download />
          </IconButton>
        </Link>
      </div>
      ...
      <Link href={release.downloadUrl}>
        <Button
          fullWidth
          color={release.isLatest ? "success" : "error"}
          variant="contained"
        >
          <Download sx={{ mr: 1 }} />
          Download, availaible only for
          <AndroidRounded sx={{ mx: 2 }} />
        </Button>
      </Link>
    </Box>
  );
...
function StyledMarkdown(props: { body: string }) {
  ...
  return (
    <Markdown className={clsx(isDarkMode && "prose-invert", "prose")}>
      {props.body}
    </Markdown>
  );
}
